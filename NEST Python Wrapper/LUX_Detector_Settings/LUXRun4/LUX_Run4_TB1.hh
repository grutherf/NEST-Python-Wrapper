...
double g1 = 0.1043; //phd per S1 photon in liquid at dtCntr (not phe)
...
double FitEF ( double xPos_mm, double yPos_mm, double zPos_mm ) { // in V/cm
  ...
  return 126.120812197 - 1.90341778291*zPos_mm + 0.0221484569642*pow(zPos_mm, 2.0) - 9.87866796872e-5*pow(zPos_mm, 3.0) + 2.0828842974e-7*pow(zPos_mm, 4.0) - 1.57249357722e-10*pow(zPos_mm, 5.0);

}

 // Ionization and Secondary Scintillation (S2) parameters

double g1_gas = 0.08845; //phd per S2 photon in gas, used to get SE size
double s2Fano = 0.; //Fano-like fudge factor for SE width
...
double E_gas = 7.5; //field in kV/cm between liquid/gas border and anode
double eLife_us = 735.; //the drift electron mean lifetime in micro-seconds
